"""PostgreSQL acceptance tests for the single Alembic schema lifecycle."""
...
class DatabaseMigrationTests(PostgresTestCase):
    ...
    def test_registry_is_unique_and_alembic_upgrade_is_repeatable(self) -> None:
        ...
        first_tables = set(inspect(database.engine).get_table_names())
        ...
        self.assertIn(
            ...
            {item["name"] for item in inspect(database.engine).get_foreign_keys("learning_sessions")},
        )
        self.assertIn(
            "fk_assignments_assignment_plan",
            {item["name"] for item in inspect(database.engine).get_foreign_keys("assignments")},
        )
        self.assertEqual(set(inspect(database.engine).get_table_names()), first_tables)
        with database.engine.connect() as connection:
            second_counts = {
                table: connection.execute(text(f'SELECT COUNT(*) FROM "{table}"')).scalar_one()
    ...
    def test_partial_legacy_schema_is_adopted_and_legacy_data_is_preserved(self) -> None:
        ...
        with database.engine.begin() as connection:
            connection.exec_driver_sql(
                "CREATE TABLE lesson_publications (publication_id VARCHAR(64) PRIMARY KEY, title TEXT NOT NULL, source_upload_id TEXT, lesson_ids_json JSONB NOT NULL, status VARCHAR(32) NOT NULL, version INTEGER NOT NULL, revision_of VARCHAR(64), created_at FLOAT NOT NULL, updated_at FLOAT NOT NULL)"
            )
            connection.exec_driver_sql(
            ...
            connection.exec_driver_sql(
                "CREATE TABLE mistake_items (mistake_id VARCHAR(64) PRIMARY KEY, learner_id VARCHAR(128) NOT NULL, source_filename TEXT NOT NULL, content_type VARCHAR(255) NOT NULL, source_image_path TEXT NOT NULL, source_image_url TEXT NOT NULL, question_payload_json JSONB NOT NULL, guide_cards_json JSONB NOT NULL, ocr_run_json JSONB NOT NULL, model_run_json JSONB NOT NULL, original_answer TEXT NOT NULL, subject VARCHAR(80) NOT NULL, grade_band VARCHAR(80) NOT NULL, chapter TEXT NOT NULL, knowledge_point TEXT NOT NULL, error_reason VARCHAR(32), notes TEXT NOT NULL, status VARCHAR(32) NOT NULL, created_at FLOAT NOT NULL, updated_at FLOAT NOT NULL, confirmed_at FLOAT, ai_error_reason VARCHAR(32), ai_error_reason_confidence FLOAT)"
            )
            connection.exec_driver_sql(
                "CREATE TABLE variation_exercises (variation_id VARCHAR(64) PRIMARY KEY, strategy VARCHAR(32) NOT NULL)"
            )
            connection.exec_driver_sql(
                "INSERT INTO lesson_publications VALUES ('paper','paper',NULL,'[]','published',1,NULL,1,1)"
            )
            connection.exec_driver_sql("INSERT INTO learning_sessions VALUES ('session','learner','paper',1,1)")
            connection.exec_driver_sql(
                "INSERT INTO exercise_attempts VALUES ('attempt','session','question','一次函数','correct',2)"
            )
            connection.exec_driver_sql(
                "INSERT INTO mastery_states VALUES ('learner','一次函数',0.2,1,0,1)"
            )
            connection.exec_driver_sql(
                "INSERT INTO mistake_items VALUES ('mistake','learner','x','text','','','{}','[]','{}','{}','','数学','初中','章节','知识点','concept','','unmastered',1,1,NULL,'reading',0.8)"
            )
            connection.exec_driver_sql(
                "INSERT INTO variation_exercises VALUES ('variation','concept')"
            )

        command.upgrade(alembic_config(database_url), "head")
        with database.engine.connect() as connection:
            self.assertEqual(
                connection.execute(text("SELECT COUNT(*) FROM mastery_states_legacy")).scalar_one(),
                1,
            )
            self.assertEqual(connection.execute(text("SELECT COUNT(*) FROM mastery_states")).scalar_one(), 1)
            self.assertEqual(
                connection.execute(text("SELECT COUNT(*) FROM mistake_attributions")).scalar_one(),
                2,
            )
            self.assertEqual(
                connection.execute(
                    text("SELECT source, category, confidence FROM mistake_attributions ORDER BY source")
                ).all(),
                [("ai", "reading", 0.8), ("self", "concept", 1.0)],
            )
            self.assertEqual(
                connection.execute(text("SELECT attribution_source FROM variation_exercises")).scalar_one(),
                "unknown",
            )
            self.assertEqual(
                connection.execute(text("SELECT publication_id FROM exercise_attempts")).scalar_one(),
                "paper",
            )
    ...
    def test_current_v027_schema_without_version_is_adopted(self) -> None:
        ...
        create_registered_schema(database.engine)
        self.assertNotIn("alembic_version", inspect(database.engine).get_table_names())
        ...
        self.assertNotIn("mastery_states_legacy", inspect(database.engine).get_table_names())
    ...
    def test_realistic_legacy_gap_shape_is_completed_idempotently(self) -> None:
        ...
        preflight = schema_report(database.engine)
    ...
    def test_schema_report_distinguishes_unversioned_database_and_missing_columns(self) -> None:
        ...
        create_registered_schema(database.engine)
        unversioned = schema_report(database.engine)
        ...
        missing = schema_report(database.engine)
    ...
    def test_health_reports_schema_out_of_date_before_business_queries(self) -> None:
        ...
        store = AppStore(database.database_url, self.data_root)
