          ...
          {payload.review && (
            <details className="review-details">
              ...
              {payload.review.text.corrections.map((item, index) => (
                <p key={`${item.field}-${index}`}><b>{item.field}</b>：<RichText text={item.original} /> → <RichText text={item.corrected} />（<RichText text={item.reason} />）</p>
              ))}
              {payload.review.vision.correctAnswer && <p><b>视觉判定答案：</b><RichText text={payload.review.vision.correctAnswer} /></p>}
              {[...payload.review.text.issues, ...payload.review.vision.issues].map((issue, index) => <p key={`${issue}-${index}`}>⚠ <RichText text={issue} /></p>)}
              {payload.question.visualContext?.map((context, index) => <p key={`visual-${index}`}><b>题图理解：</b><RichText text={context.description} /></p>)}
            </details>
          )}
          {payload.question.givens.length > 0 && (
            <div className="givens" aria-label="题目条件（辅助读题）">
              <span className="givens-heading">题目条件</span>
              {payload.question.givens.map((given) => <span key={given}><RichText text={given} /></span>)}
            </div>
          )}
        </div>
