...
上传完成接口现在返回 `202` 和 `jobId`，Hook 为每个文件独立轮询 `/api/jobs/{jobId}`，并行展示排队、处理中、
完成、失败或取消状态。列表组件不需要等待某一个 PDF 完成；条目级轮询适配器还提供取消和有限重试入口。
...
- 当前 `payload`。
- 整本生成完成前的临时 `questionBank`，完成后替换为整卷题库。
- 当前结构化答案和自由文本。
- 提示等级与内容质检预览结果。

每个 PDF 上传完成后由后台 Worker 按五页批次持续生成整本教材；页面可以显示首批进度，但不再要求用户手动逐批点击。
`processPdfBatch()` 仍保留给已完成教材的单批修复、重新 OCR 和局部补跑。
