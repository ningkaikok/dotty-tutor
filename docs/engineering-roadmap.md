...
- [ ] 题号识别根治（`QUESTION_START_PATTERN` 只认行首，`backend/domain/questions/source.py:13`）。
  ...
  **子问题 B：题号完全没有换行、直接粘在上一题末尾（假阴性）。** 真实坏样本：
  “D. 一、三象限9. 如图，在RtΔABC中…”——“9.”不在行首，`QUESTION_START_PATTERN` 根本不会尝试匹配它，
  这道题（以及跟着粘连的第 10 题）从未被识别成独立区块，整题消失，数据库里题号直接从 8 跳到 11。
  **方案**：需要在行内（不只是行首）扫描题号候选，再用启发式规则排除误伤——比如要求候选数字后面紧跟
  典型题干开头词（如图/一个/求/计算/设/已知）或选项字母序列，同时排除坐标、百分数、页码引用等常见
  数字场景。**难度：高**——扫描范围从“行首”扩大到“任意位置”，误判面显著增大，且这类启发式规则很难
  在写代码时就确认覆盖完整，必须先有金标准集/Badcase 语料才能有把握改，不要在没有回归网的情况下动手。

  **建议顺序**：先做 `feature/badcase-replay-loop`，把这两个真实坏样本连同后续陆续发现的同类案例
  一起收进语料；子问题 A 风险可控，可以在语料建好后较快跟进；子问题 B 等语料积累到能覆盖常见数字
  场景（坐标、百分号、页码引用等）之后再动手，不要抢跑。
- [ ] 图片与题目的版面级对应关系：图片归属由 MinerU 的线性阅读顺序决定
  ...
  （见 `_apply_caption_image_attribution`）。仍未覆盖的是“既没有内联图注、也没有显式题号标注”的
  纯位置场景，需要版面坐标或语义校验才能根治，属于独立工程，不要与渲染契约混在一起做。
  > 优先级：低。两条高置信度信号（内联图注、显式题号标注）已经覆盖大多数真实案例；剩下的纯位置场景
  > 目前没有独立于子问题 B（题号识别）之外的确认坏样本——先看 Badcase 语料积累后这类场景的真实占比。
...
T0 里剩下的几条开放项（题号识别子问题 A/B、图片版面纯位置场景、`MathText` 讲解通道、`subQuestions`）
分别值不值得单独立项，用语料里的真实占比决定顺序，不要凭感觉排期。
