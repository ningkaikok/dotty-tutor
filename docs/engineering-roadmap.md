...
- **T3 实验能力**：主动跟踪新模型、新库和新基础设施，但先以可回滚、可比较的实验形式验证，
  不把未经评测的能力直接接入生产学习状态。

## 最高优先级：AI 工程方向（2026-08-31；v0.31.0 已收口）
...
本节的 AI 能力实验不以真实试用信号为前置，也不再等待产品需求触发。项目会主动跟踪新模型、多模态、
工具调用、Agent、检索、语音和本地推理方向，并在本地脱敏或合成数据上快速启动小范围对照。每个实验仍必须
...
1. **陪练上下文分层（切分与度量已完成，2026-09-05；缓存未启用）**：陪练提示词按
   ...
   组成动态后缀。切分由 `PromptParts` 值对象承载：`text` 由 `stable + dynamic` 拼出，
   因此“稳定段是整段提示词的字面前缀”是结构性保证，不需要在调用边界校验。
   `model_call_metrics` 新增可空的 `stable_prompt_chars` / `dynamic_prompt_chars`
   （迁移 `0007_prompt_prefix_split`），聚合新增 `stablePromptShare` 与
   `prefixSplitCoverageRate`。
   > **实测占比（三轮真实结构的陪练线程，指示性）**：稳定段 961 字符恒定不变，
   ...
   > 动态段末尾补了一句收束指令。这是一次真实的提示词结构改动，而现有评测覆盖的是
   > 讲解质量、不覆盖陪练回复质量——这次改动的效果**没有被自动化手段验证过**。
...
3. **评测语料继续扩充（已完成，2026-09-05）**：`EXPLANATION_SAMPLES` 由 8 条扩到 16 条，
   ...
   > 整体测不出来（4/6 被强烈标记为错误，家族整体 factual 均分反而低于算错类错误），而是
   > 特定一种子类型——**去掉了一个限定条件的真实规则过度推广**（SSA、反比例函数象限）——
   > 两次独立运行都被漏检；"独立虚构、可被单个反例推翻"的断言两次都被抓到。这个更窄的假设
   > 样本量仍只有 2，还需要更多同类样本才能确认。详见
   > [`模型与系统测试报告`](model-evaluation-report.md) 第 10.7–10.9 节。

三项的当前状态：第 2、3 项已完成；第 1 项的切分与度量已完成，是否启用缓存待真实数据
与 Provider 支持确认后再决定。学习目标阶段性完成后，回到下面“当前执行队列”从头继续，
不需要重新排序。

## 当前 AI 能力实验路线（2026-09-14）

项目的主要目标是主动跟踪 AI 工程和模型能力演进，因此新增实验路线不等待真实老师试用或产品需求触发。首个实验已从
`feature/multimodal-tutor-input` 完成，并保持为一条可独立删除的垂直切片：

1. **统一多模态输入（已完成）**：定义 `TutorInput`，承载学生文字、题图裁切、画板快照、公式识别结果和结构化答案；
   增加独立观察适配器，输出观察事实、置信度和证据区域。原图、识别文本、模型观察和学生修正分层保存，
   低置信度结果必须先由学生确认。
2. **受约束工具调用（已完成 shadow 版）**：在现有 `Tutor Turn Plan` 和状态机上增加类型化 `ToolProposal`，覆盖
...
3. **Tutor 专用评测实验室（第一版已完成）**：扩展现有离线语料，覆盖错误定位、苏格拉底提问、
   提示升级、图片理解、策略遵守、工具越权、延迟和调用成本。评测应同时保留规则指标、结构指标和独立 Judge，
   不以最终答案正确率代表教学质量。
4. **交互数学画布（最小版已完成）**：作为多模态 Tutor 的结构化输出表面，先支持点放置几何场景；画布动作进入
   `interactionSpec` 和 `answerSpec`，不让模型直接操作学习状态。它是 AI 输出与学生操作之间的实验，不是先做动画库选型。
5. **检索实验（PostgreSQL 第一版已完成）**：如果要追踪 RAG，先做教材片段的 PostgreSQL 全文检索和页码/版本证据，不先引入向量数据库；
   在每个检索实验周期主动比较全文检索、混合检索、reranker 和 embedding；是否进入产品仍由统一评测、
   证据可追溯性、延迟和成本共同决定。
...
本项目新增一条与产品 AI 主线并行的实验线，用于持续跟踪模型能力和 AI 工程方法的变化。
它不承诺把每个热点都做成产品，也不把实验性依赖直接带入学生学习状态或内容发布链路。
每项实验都必须先有脱敏/合成基准集、可比较的基线、成本与延迟记录、shadow 或只读运行方式，
并能独立回滚。

当前候选方向按建议顺序排列：

1. **模型路由与推理质量**：比较不同模型在 Tutor 的错误定位、苏格拉底提问、视觉观察和工具提案上的
   质量/延迟/成本，先扩充跨模型统计横评，再决定是否引入按任务能力路由。
2. **Agent 轨迹与工具安全**：主动评测“模型提出了什么、系统拒绝了什么、最终执行了什么”，继续保持
   `ToolProposal` 和确定性状态机边界；LangGraph 等编排框架先在隔离实验中比较，不直接替换生产状态机。
3. **长期学习者上下文**：在现有题目快照、线程摘要和证据链之上，实验有限生命周期的知识点画像、
   错误模式和提示依赖，不保存无边界聊天记忆。
4. **检索增强 Tutor**：以 PostgreSQL FTS 为基线，使用评测结果决定是否增加混合检索、reranker、
   embedding 或向量索引；检索结果必须保留教材版本、页码和证据引用。
5. **语音与实时多模态**：评估语音输入、语音输出和实时 Tutor 的教学收益，先做离线录音/转写回放，
   不直接改变现有同步状态机和学生证据模型。
6. **本地推理与成本治理**：评估 WebLLM/WebGPU、量化模型、Prefix Cache 和更细粒度的降级策略，
   仅在真实基准证明隐私、离线或成本收益后推进。

第一批建议执行顺序是：扩充 Tutor 跨模型评测 → Agent 轨迹/工具安全评测 → 学习者画像实验 →
检索增强对照 → 语音或本地推理实验。所有实验仍遵守“模型不能直接修改掌握度、发布状态或学生原始证据”的约束。
...
- [x] **符号等价判题**：`answer_evaluator.py` 目前的归一化覆盖了教材最常见的格式差异
  ...
  **不改变现有分层**：结构化归一化优先，符号层只处理归一化判否的样本，解析失败仍返回 `None`
  ...
  > 已完成：`check_answer_agreement` 同时服务核验和学生结构化作答；表达式答案支持明确的花括号、顶层
  > “或/or/分号”和同一变量重复赋值逗号解集，按元素文本→数值→符号三态一一匹配，重复元素按集合语义处理。
  > `answer_evaluator` 只有候选 agree 才判对，全部 disagree 才判错，否则返回 undecidable；确定性学习写入会阻断
  > 客户端伪造的判定。该实现不解方程，也不新增 `answerType`，沿用已有 `expression`/`numeric` 文本契约。
  > 注意延迟：学生每次提交都会走这条路，而核验阶段是离线批量，两者的成本约束不同。

- [x] 图片与题目的版面级对应关系：图片归属曾经完全由 MinerU 的线性阅读顺序决定
  ...
  > 已补齐安全闭环：结构化/显式图注优先于 bbox，bbox 会检查已经线性绑定的图片；高置信唯一候选可纠正错绑，
  > 低置信、候选接近、相对路径/同 basename 歧义、无独立题号 bbox 时移除旧绑定并写入 `needs_review` 审计，整卷
  > 质量报告阻断 `readyForFullPaper`。审计新增 previous owner、候选/分数、最终来源和 abstain reason，同时兼容旧字段。
  > 置信阈值当前为最低 `.72` 且候选间隔 `.12`；仓库没有已确认的纯位置真实坏样本，真实阈值回放校准仍待后续语料，
  > 因此本条不宣称根治所有真实版面坏样本。
...
- [x] 在模型调用边界记录 provider、model、task（生成/审核/陪练/陪练评测/OCR）、耗时、token 用量、失败和
...
- [ ] 主动选择一个互动数学库做小范围实验（Mafs 或 JSXGraph），由 `InteractiveMathCanvas` 隔离。
- [ ] 主动评估 Manim、Motion Canvas 和 Lottie 的代表性场景；实验结果只进入表现层，不承载学习状态。
- [x] RAG 第一版先使用 PostgreSQL 元数据/全文检索；后续主动对照混合检索、reranker 和向量检索。
- [ ] 主动评估 TTS 音频缓存、对象存储和 CDN；先用合成负载比较延迟、成本和可靠性，再决定生产化范围。
...
- [ ] 版本化轻量知识点树（`id`/`name`/`parent_id`/`taxonomy_version`/`status`），作为 P2 数据治理实验。
  ...
  主动做轻量派生索引和先修关系实验；正式图谱、embedding 或向量基础设施仍必须经过可重建性、准确率和
  错误放大风险对照，不因为实验结果就直接写入学习状态。
...
- 暂不引入全局 LangChain/LangGraph、Multi-Agent、独立 Control Plane、Redis/Kafka 或 Tool Gateway；
  这些方向可以在 AI 前沿实验线中主动做隔离评测，生产采用仍需通过轨迹、安全和可靠性门禁。
