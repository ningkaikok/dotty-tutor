...
- [ ] 题号识别对“题号紧跟上一题末尾、OCR 没有换行”的排版失效（`QUESTION_START_PATTERN` 只
  认行首），会把多道题合并成一个区块，导致题目彼此消失或被覆盖。已加安全网防止静默覆盖
  （见 PR fix/duplicate-question-key-safety-net），但被合并吞掉的题目本身不会自动恢复，
  需要更可靠的题号边界识别（不能只看行首，需要结合上下文如“上一行是否以完整句末标点结束”）
  才能根治，属于独立工程。
