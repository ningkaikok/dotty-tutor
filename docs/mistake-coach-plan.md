...
- 建立 versioned policy 复习任务和学习进度页。（已完成；缺少策略元数据的旧任务使用 `unknown:legacy` 的 legacy 1/3/7 天兼容行为）

前两步已将验证题作为独立练习记录保存，而不是混入自由对话消息。生成服务根据六类错误原因选择策略；
验证题只生成一次，重复请求复用已保存题目，答错允许更新同一条作答证据，答对后记录原子迁移为
`mastered`。系统只接受可确定性判题的选择、多选、填空和数值题。迁移时按题目/知识点的 versioned policy
幂等创建带 `scheduleVersion`、`sequenceNo`、`profile` 和 `supersededAt` 的任务；缺少策略元数据的旧任务使用
`unknown:legacy` 的 legacy 1/3/7 天兼容行为。每项任务保存独立迁移题和一次作答结果，进度页汇总掌握率、到期任务、完成数、复习正确率和知识点状态。
...
多模态输入已在 `feature/multimodal-tutor-input` 的 v0.31.0 切片中接入：图片与画板首先转换为带置信度的观察事实，
学生确认后才进入教学决策；公式候选同样默认待确认。这样可以避免把 OCR 或视觉模型的猜测当成学生真实思路，
也能让错误结果被定位、重试和审计。当前画布先限定为可回放的 `math-canvas-v1` 点放置场景，复杂手写公式语义仍未实现。
