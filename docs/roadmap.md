...
拍照录入、多轮陪练、掌握验证和 versioned policy 复习（旧数据保留 legacy 1/3/7 天兼容行为），但尚未达到匿名公网服务所需的安全性、隔离性和
可运维性。
...
- API 没有登录、授权、用户或租户隔离；班级、成员和作业指派已按本地单机单库模型落地。
...
- 错题陪练已实现对话线程、受约束状态机、按错误原因生成的变式验证、掌握复习闭环和第一版按目标类型的复习 policy/gate；多轮重新掌握仍未实现。
...
- [x] 使用错题本、变式题、掌握验证和 versioned policy 复习任务实现学习闭环的基础版本；旧任务保留 legacy 1/3/7 天兼容行为。
...
"录入一题 → 陪练 → 验证掌握 → 复习"闭环已交付，交付记录见
[AI 错题陪练产品规划](mistake-coach-plan.md)。剩余开放项：真实移动设备与微信内浏览器的弱网体验验证；当前队列隔离和恢复不等于 PWA。
...
- [x] 移动端布局、键盘操作、错题后台取消/重试和归档确认已补齐第一轮；仍需结合真实移动端试用验证上传、压缩、权限和弱网体验，当前不等于 PWA。
...
- [x] 建立 shadow 学习者画像第一版，仅聚合错误模式、提示依赖、最近练习和已有掌握证据；事实带 evidenceRef/生命周期/publication scope，不保存无边界聊天历史，不注入生产掌握度。
...
这一阶段优先复用现有教材、课程、学习会话和题型组件，不另建一套试卷渲染器。生产级教师后台（登录、权限、
多租户、跨班级协作）和复杂排课继续放在受控公网测试之后；当前单机单库的班级、作业和教师看板已落地。
