...
flowchart TD
  ...
  Q --> R[统一审核模型：文字 + 题图]
  R --> S[重新绑定来源图片]
  S --> T[公式、选项、contentBlocks 规范化]
  T --> U[题目质量门禁]
  U -->|ready| V[保存 lesson + question revision]
  U -->|needs_review| W{还有有限重试?}
  W -->|是| N[重新执行当前题的阶段编排]
  W -->|否| X[隔离题目，不发布]
```
...
四个生成阶段与审核模型之间的职责是严格分开的：抽取模型可以提出结构候选，但不能覆盖 OCR 来源事实；求解模型
...
```text
OCR 题块 + 来源图片
  → ExamIR/QuestionIR 固化题号、题干边界、块 ID、页码和图片 ID
