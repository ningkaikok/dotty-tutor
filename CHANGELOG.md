# 变更记录

本文件记录项目的功能、接口、模型、交互和工程变更。格式参考 Keep a Changelog，版本号遵循
Semantic Versioning。

## [Unreleased]

后续改动写入此区域，发布时再整理到具体版本。

### Added

- 内容工作台新增陪练模型切换前的 50 条合成草案对照；可查看结构输出、逐维度指标、延迟、Token 和逐题结果，只有评测结束后才可应用候选模型。学生端继续使用服务端当前模型，草案不计入正式人工金标准。
- 新增 50 条合成评测候选草案及逐条人工审核清单，供审题、修订和复核使用；草案不计入正式人工金标准。

### Fixed

- 最后一题答对后，先展示作答反馈并允许手动查看完成结果，避免完成页立即覆盖反馈。
- 内容生产端合并练习序号与教材原题号的主次展示，避免两套编号造成理解负担。

## [0.32.0] - 2026-09-23

### Added

- 答案核验阶段的 `solverAgreement` 不再由模型自我声称，改由确定性符号判等程序裁决：解答与来源答案判定为不一致时强制题目进入冲突状态并要求人工复核，判不了的开放题（证明、文字作答）明确记为未核验，不会被当成核验通过。
- 整卷生成连续遇到 API key 过期/配额耗尽、限流或上游超时时会提前暂停剩余批次并给出具体原因，不再把剩下的题逐一磨成失败记录；已经成功的批次不受影响。
- 新增只读的环境依赖自检接口 `GET /api/system/dependency-preflight`，一次性检查 MinerU、pypdf、Ollama、Codex CLI、Azure Speech、Qwen3-TTS 和 PostgreSQL 是否配置就绪；内容生产端新增对应的自检页 `/studio/dependency-preflight`，入口在 `/studio` 首屏顶栏。
- 新增教师讲评清单 `GET /api/classes/{classId}/assignments/{assignmentId}/lecture-checklist?limit=5`：`limit` 约束为 1–50，按最新作答聚合共性错题、错因分布、涉及学生和 evidenceRefs，保留 unknown 错因，并返回 `attributionSource`/`mistakeEvidenceRef`，展示层应用教师最新 overturned 结果。
- 新增按目标类型选择的复习 policy/gate 元数据、定量/定性双门槛、按掌握度推进与失败回退；只有教师明确编辑的 `objectiveType`、`gateMode`、`policyVersion` 才能启用 typed policy，缺少元数据的历史任务继续使用 `unknown:legacy` 兼容策略。定性判题缺少受约束 rubric、置信度或 evidenceRefs 时保持 `needs_review`。
- 新增错题后台导入 `POST /api/mistakes/import-jobs`，支持幂等、进度查询、协作式取消和有限重试；取消请求在安全点收敛并受租约/状态保护，避免旧 Worker 覆盖终态；新增固定 ID、完全合成、幂等的 `scripts/seed_demo_bundle.py` 与 `--verify`。
- 新增金标准 JSONL 契约校验、配对统计和离线 Prefix Cache probe；正式 50 条计数只接受独立标注人与复核人的 `sourceKind=human` case，新增 shadow 学习者画像与工具安全评测基础设施。官方资料与 Luna usage 已确认 Provider 具备缓存能力，但应用前缀尚未观察到高于 hidden baseline 的新增命中；上述实验也不代表 50+ 人工金标准或跨模型统计已经完成。

### Changed

- 学生端第一轮移动/键盘体验已收口：窄屏布局、非原生画布键盘等价操作、归档确认焦点管理、弱网作答队列隔离和用户可见术语统一；弱网队列不等于 PWA。

### Fixed

- 修复确定性答案在表达式归一化失败时把开放文本误判为错误的问题；现在支持明确解集的顺序/写法等价，判不了的结果会回退而不写入伪造的 mastery 证据。
- 修复图片已被 Markdown 线性绑定后 bbox 只能补漏、不能纠正错绑的问题；显式图注优先，高置信 bbox 可纠正，低置信或路径歧义会移除旧绑定、进入审计并阻断整卷质量门禁。纯位置阈值仍待真实坏样本回放校准。

## [0.31.0] - 2026-09-14

### Added

- 统一错题陪练的 TutorInput，支持文字、结构化答案、步骤图片、公式识别候选和结构化数学画布，并对低置信度观察强制学生确认。
- 增加 Tutor 工具提案的严格契约、阶段策略和 shadow 审计；新增 Tutor 离线评测实验室与 PostgreSQL 全文检索索引。
- 增加三条 Alembic 迁移，保存多模态证据、结构化画布、工具策略审计和已发布题目的全文检索索引。

## [0.30.0] - 2026-09-12

### Added

- 新增 ExamIR/QuestionIR 来源快照、四阶段题目生成、批次阶段缓存和审核队列接口；结构化 MinerU block 缺失时明确回退并进入复核。

### Changed

- 题目生成改为原题抽取、独立求解、答案核验、教学脚本的分阶段编排；指定阶段重跑会复用上游产物并重跑下游，未核验答案不会调用教学脚本模型。

### Fixed

- Hardened equation checks and stage rerun errors against pathological input and internal stack-trace exposure.
- 错题导入、列表、详情、确认和归档接口现在统一使用学生题目白名单，避免下发阶段产物、模型运行信息、质量审核和核验字段；服务端仍保留完整题目载荷用于判题与陪练。
- 阶段重跑现在使用不可复用的 rerun token，目标阶段及下游不会误命中旧缓存；审核后的题干、题号、选项和小问重新锁回 QuestionIR，学生发布载荷也改为顶层白名单投影。
- Schema 落后于应用版本时，学习成本和模型指标接口返回可执行的迁移错误，不再被统一成无诊断信息的 500。

## [0.29.0] - 2026-09-05

### Added

- 新增模型调用边界指标的提示词稳定段/动态段字符切分（陪练提示词按稳定前缀在前重排），聚合报告新增稳定占比与切分覆盖率，为判断 Prefix Cache 是否值得启用积累数据；缓存本身尚未启用。
- 新增模型调用回退信息落库：`providerAttempts` / `schemaFallback` 此前被静默丢弃，聚合报告新增重试放大倍数与 Schema 降级率。
- 新增讲解 Judge 评测语料的事实性标注与评审区分度指标，用于验证评审模型是否真的能分辨对错讲解，而不只是彼此打分一致。
- 新增班级花名册查询接口与学生端身份选择器：修复学生端身份写死导致老师指派的作业无法送达对应学生、班级看板永远显示"未开始"的问题；仅面向单机试用场景，不是登录鉴权。

### Changed

- 陪练变式题生成校验失败时自动把错误反馈给模型重试一次，不再要求用户手动用同一份必错 prompt 反复重试。
- 上传分块限额统一到 Pydantic 契约层单一事实源，移除路由层从未生效的影子副本。
- 后端 CI 门禁改为在 Python 3.12 与 3.14 两端矩阵运行，覆盖 `requires-python` 声明的完整支持区间。

### Fixed

- 修复学生作答判定完全由客户端自报、服务端未做任何复核的问题：写入端现在会用同一套确定性判题规则重新核验，判定被覆盖时记录事件；这个问题在接入登录鉴权后依然存在，因此单独修复。
- 修复学生端题目与变式题接口下发标准答案的问题：改为按白名单投影后再返回，变式题接口同时补上归属校验。
- 修复班级个性化作业生成在并发下不幂等的问题：老师连续点击会产生重复发布的试卷。
- 修复调试端点凭据可通过 URL 查询参数传递、且未使用恒定时间比较的问题；改为只接受请求头并使用恒定时间比较。
- 修复判断题只在陪练回复里内联判定、服务端复核不覆盖判断题从而回退到客户端自报结果的问题，并入统一的确定性判题器；同时修正了内联实现里"不正确"被误判为"正确"的否定式匹配缺陷。
- 修复陪练变式题生成的题型白名单遗漏 `true-false`、导致模型生成判断题变式被直接拒绝的问题。

## [0.28.0] - 2026-09-01

### Added

- 新增多模型评审排行榜：可分别比较多个评审模型的一致性，以及在固定评审模型下比较候选生成模型的讲解质量；报告明确标注小样本不代表统计显著性。
- 新增 Alembic 统一数据库迁移链、schema registry、预检/升级/验证命令，以及 append-only 错因归因历史表和历史列回填。

### Changed

- PostgreSQL 运行时、Worker 和业务脚本继续只通过显式迁移管理 schema；Store 不执行 DDL，schema 落后时健康检查返回明确的 `503 + SCHEMA_OUT_OF_DATE`。
- Docker Compose 在启动 API 和 Worker 前通过一次性 `db-migrate` 服务升级 schema；迁移失败会阻止业务服务启动。
- 后端数据库测试统一使用隔离 PostgreSQL：普通 Store 测试按测试清理业务表，adoption/legacy 测试使用未迁移空库，CI 与本地 runner 采用同一套流程。
- 移除数据库 URL、Upsert、Store 初始化和任务领取中的旧兼容路径；`DOTTY_DATA_DIR` 仍只决定文件资产目录。

### Fixed

- 修复 Judge 与生成契约缺少严格 JSON Schema 限制、导致 Codex 结构化输出调用被拒绝的问题。
- 修复 PostgreSQL-only 模式下 OpenAPI 类型导出依赖运行时数据库配置，以及全新 Compose 数据库未先迁移便启动 API/Worker 的问题。

## [0.27.0] - 2026-08-31

### Added

- 新增班级级个性化作业 MVP：教师可基于脱敏班级证据生成一份全班共享的新试卷，生成结果单独发布、可审阅后确认指派，并支持幂等重试。
- 新增变式题归因来源快照：统一记录采信的 AI、自评或 unknown 来源，历史列表和 Evidence API 保持生成当时的归因。
- 新增教师看板复核闭环：老师可复核或推翻单题判定、覆盖知识点掌握度，并查看按作业聚合的复核率与推翻率；原始学习证据保持不变。
- 新增计划式作业布置：教师先选择班级和已发布试卷生成脱敏分析草稿，审阅班级目标、证据、覆盖度和提醒后才创建正式作业；计划确认支持 stale 校验和幂等重试。
- 新增班级、作业指派和教师掌握度看板；教师可维护班级名单、指派已发布试卷，查看作业完成情况与知识点分布，学生首页按指派作业展示任务。
- 新增脱敏金标准集、Badcase 登记簿、确定性回放与 LLM-as-Judge 评测闭环，支持比较模型输出并保留修复依据。
- 新增错题学习证据链：自动错题先由学生确认错误原因，变式验证保留每次作答证据，并提供验证正确率、复习完成率和同知识点再错率。
- 新增单道错题证据查询接口，串联错误原因、变式策略、掌握转变和 1/3/7 天复习任务。
- 新增学习效果与模型成本联合报告，展示学生累计漏斗、全局模型调用失败率/耗时和 Token 覆盖率；报告明确不估算货币成本，也不做学生级归因或因果推断。

### Fixed

- 修复确认错题时省略或传入 `null` 会意外清除已有学生自评的问题；首次未填写仍保持为空，非法错因枚举继续被拒绝。
- OCR 题号切分失败时，含多个题目候选的整卷不再回退成"整页当作一道题"送给模型，而是在来源边界返回
  422 并提示刷新 OCR。此前该回退会把答题说明、多道题和参考答案拼成一道看似合法的题，下游无法发现。
- 防止验证题重复提交或跨题复用 `attemptId` 时重复写入或错误关联学习证据。

## [0.26.0] - 2026-08-30

### Added

- 知识点掌握度升级为按发布版本实体和最新不同题证据计算的 mastery-v2；作答响应新增知识点 ID、原始分、证据数、置信度、算法版本和计算时间，支持可重复数据库迁移。
- 教材导入完成后生成质量报告，展示预计题数、题号范围、重复题号、未识别页和图片归属冲突；存在阻断项时暂停整本模型生成。
- 生成和审校模型使用稳定的图片占位符，图片引用丢失或乱序会被确定性门禁拦截，避免依赖提示词维持题图版面。
- 多小问题支持独立题型、答案和判定状态；确定性小问可分别反馈，开放性小问继续通过陪练处理。
- 教材导入质量报告增加图片坐标归属审计；无法高置信判断时会进入人工复核，避免相邻题目图片静默错绑。
- 新增安全富文本渲染，课程讲解、陪练历史和错误回退中的普通文字、换行与显式公式可以一致显示。

### Fixed

- 修复 OCR 使用全角逗号标记题号时未能切分（例如“17，”），导致整页题目被错误合并并重复生成同一道题的问题；强制重跑批次会清除旧的整卷完成标记。
- 修复多小问作答中开放性小问误进入客观掌握度，以及确定性答错证据被丢弃的问题。
- 修复学生学习入口初始化请求重复和本地回归测试并行冷启动导致的状态不稳定。
- 修复题图引用和题目质量门禁相关的导入回归。

### Changed

- 学生端多小问答案支持刷新、切题和离线补传后恢复；本地 Playwright 回归默认使用单 worker，CI 仍保留并行配置。

## [0.25.0] - 2026-08-23

### Added

- **模型调用边界指标**：每次模型调用记录 runtime/task/provider/model/耗时/token 用量与失败，聚合经 `GET /api/metrics/model-calls?days=N` 查询；内容生产端新增"模型调用指标"面板（最近 7/14/30 天）。
- **LLM-as-Judge 讲解质量评估基建**：固定 rubric（清晰度/针对性/事实性，各 1-5 分）+ 版本化提示词；输出校验门禁拒绝分值越界、缺依据与置信度越界的评审；按需 CLI `python -m evaluation.judge_cli` 生成评分报告。
- **开发期只读状态报告**：`python -m evaluation.report` 一页聚合 Badcase 登记簿状态、确定性重放计数与 Judge 评审均分；严格只读。

### Fixed

- 修复数据目录默认锚点在 monorepo 布局下指向 `apps/data` 的问题——现在稳定落在仓库根 `data/`。
- 修复 Store 在 engine 与 database_url 同时缺失时仍调用 create_engine 的潜在崩溃（改为提前抛出明确错误）。
- 修复 OpenAPI 类型导出步骤在无 PostgreSQL 的 CI 环境失败的问题（指标存储改为惰性建表）。

### Changed

- HTTP 路由层扁平化：`apps/api/api/routers/ → apps/api/routers/`，消除双层 api 嵌套。
- 移除范围声明式的 requirements 文件：Docker、CI 与本地统一使用 uv 精确锁安装（依赖唯一来源为 `apps/api/pyproject.toml`）。

## [0.24.0] - 2026-08-23

### Added

- 新增学习效果漏斗快照接口 `GET /api/funnel`：聚合错题导入 → 人工确认 → 开始陪练 → 变式验证 → 复习完成五个阶段的计数与比率，分母为零时比率为 null 而不是 0。
- 陪练的确定性判题现在携带客观判定证据（哪些空未匹配、数值作答原文与容差、已选选项等）随回复和消息动作持久化；数值题空提交时会自动引导回题干条件而不是提示"检查计算"。

### Changed

- 仓库结构迁移为标准 monorepo 布局：`backend/ → apps/api/`、`frontend/ → apps/web/`，前端改用 pnpm workspace 管理；本地开发入口不变（59174 / 8010），Docker 构建路径已同步更新。克隆旧书签或脚本若引用原目录需要调整。

## [0.23.0] - 2026-08-22

### Added

- 教材导入现在会在正式 OCR 前对每一页做轻量预检分类（空白页、出版信息页、公式密集页、图文混排页等），并在运行记录中给出脏页摘要；空白页不再浪费版面 OCR 成本，页面本身不会被删除或跳过审校。
- 模型目录现在携带能力元数据：每个模型标注支持的角色（生成/审核/陪练/视觉）、能力标签（结构化输出、视觉、数学、长上下文）、上下文上限、延迟与成本级别和回退建议；连续调用失败会记录健康状态用于候选筛选提示，成功后自动复位。
- 新增离线评测基建：脱敏金标准语料、坏样本登记簿（统一标签 + 修复证据约束）和确定性重放器，切分规则等变更前后可自动对比结构差异；题号切分修复的回归证据已入集。

### Fixed

- 修复 OCR 在句子中间插入段落断行时，"3、4." 这类续举例编号被误判成新题号、进而与真正的第 3 题共享编号互相覆盖的问题；受影响教材的题号序列恢复干净，此前丢失归属的题目插图也回到正确位置。

## [0.22.0] - 2026-08-22

### Added

- MinerU 解析教材时同时产出的 `content_list.json`（块级结构，含图片/表格/图注）和 `middle.json`（行级结构，含精确 bbox）现在会随 `source.md` 一起落盘保存，不再被直接丢弃；本次改动本身不改变任何现有行为，只是把数据留下来供后续图注归属和换行重建复用。
- 图片/图表/表格的"第N题图/第N题"归属现在优先读取 `content_list.json` 里 MinerU 已经识别好的结构化图注字段，而不是只在扁平 OCR 文本里用正则去猜；没有结构化数据时完全回退到现有的正则逻辑，行为不变。

### Fixed

- 修复题号紧跟上一题末尾、没有任何换行导致题号本身在切分阶段完全消失的问题：现在会用 `middle.json` 的行级坐标重建被扁平 Markdown 吃掉的换行，再交给现有的题号识别规则处理，规则本身不变；没有 `middle.json` 时完全回退到现有的扁平文本路径。

## [0.21.1] - 2026-08-21

### Fixed

- 修复同一批次内两道题目因 OCR 排版把题号误判成新题而共享同一个存储 key、导致后写入的题目静默覆盖前一道题的问题；重复题号现在会被隔离为独立候选并标记为待人工核对，而不会覆盖任何已生成的内容。
- 题干中的图片现在显示在它在题目里出现的位置，而不是全部堆在文字末尾；“主视图”“俯视图”“图1/图2”等图注能与对应图片对上。
- OCR 原文里图片后紧跟“第N题图/第N题”这种明确标注时，图片归属现在优先信任这个标注而不是纯文本位置，修复题号识别失败时把无关图片错绑给相邻题目的问题。
- 修复图片选择题清理占位文字时，只删掉裸标记行、留下“A（数轴图）”这类带内容的占位文字残留在题干里的问题；现在整行占位文字都会被删除。
- 修复图片选择题清理占位选项文字时，把几何题干里以裸字母开头的正常内容（如“AB是弦”）连同后半行一起误删的问题。
- 修复三选项题的题干与结构化选项重复显示同一组内容：重复检查此前只认恰好 A-D 四项，现在按实际选项正文逐项比对。
- 修复图片选择题被质量门禁误判为“题干中重复包含结构化选项”的问题：题干清理干净、结构化选项是裸标签时，两侧比较会凑巧撞上相同的空字符串。
- 不参与确定性判题的题型（如简答、证明）若携带只覆盖某一小问的答案规范，发布前会被判为待人工复核，避免这类静默错配在题型调整后变成误判；含多个小问的题目会给出提示。

## [0.21.0] - 2026-08-21

### Added

- 题干中的统计表（如阅读量分布表）现在会被解析成可正常显示的表格，不再把原始 HTML 标签当文字展示给学生；错题确认改写题干后同样生效。
- 教材上传完成后自动开始整本生成，仍保留分批断点续跑和安全上限。

### Changed

- 整卷生成不再重复处理已完成的批次，减少重复的模型与审校调用。

### Fixed

- 修复内容生产端“修复本题”对每个批次第 6 题之后的题目一律失败、提示“OCR 结果中已找不到这道题”的问题。
- 修复题干把图片写成 `[主视图图片：images/xxx.jpg]` 这类方括号注释时，只删掉路径、留下残缺 `[主视图图片：` 前缀的问题；整段注释会被删除，图注文字保留。
- 修复普通题目的题干把图片路径显示成文字的问题：图片引用清理此前只对 A-D 图片选择题生效，现在对每一道题生效；同时新增发布前的确定性检查，任何残留都会让题目进入人工复核而不是原样展示给学生。
- 课后练习的题目序号改为“已生成 X/Y 题”，并为原题题号增加说明，避免把“本次会话已生成的第几题”误认成与教材原始题号不一致的错位。
- 整卷任务刚排队、Worker 尚未写入首个进度快照时，进度接口不再返回 404，轮询状态保持稳定。
- 修复多文件教材上传时并行轮询互相覆盖进度，恢复合并、校验、OCR 和生成阶段的细分进度，避免进度条跳动或倒退。
- 修复识别任务重试时误要求已清理的上传分块，恢复使用已合并 PDF；临时 HTTP 错误由 Worker 自动重试，前端网络抖动也会自动重连。

## [0.20.0] - 2026-08-20

### Changed

- 建立全新数据基线：切换当前版本前必须清空本地测试数据库和仓库内 `data/` 资源；各领域首次访问 Store 时自动创建当前 schema，项目不提供外部或原地数据库迁移。

## [0.19.0] - 2026-08-20

### Added

- 内容生产预览支持在首批 5 题之后显式排队生成有安全上限的整套试卷，并可取消、恢复或重试任务，查看每个
  PDF 批次的成功、失败、隔离和跳过汇总。

### Fixed

- 修复首次启动 Docker Compose 时 API 与后台 Worker 并发初始化共享教材卷可能导致容器启动失败的问题。

## [0.18.0] - 2026-08-19

### Added

- 大 PDF 完成和批次识别进入可恢复后台任务，上传后可独立查询进度、取消失败任务并执行有限人工重试。

### Changed

- 模型、OCR、审核和陪练运行统一记录 Provider、模型、Prompt、Schema 与校验器快照，失败响应提供稳定错误码和请求 ID。
- Docker 部署增加独立 Worker；API 快速返回任务 ID，不再占用一个 HTTP 请求等待整批 OCR 和题目生成。

### Fixed

- 长时间教材处理不会再阻塞 HTTP 请求；任务失败、取消和 Worker 重启后都有明确状态与恢复路径。

## [0.17.0] - 2026-08-17

### Fixed

- 试卷“注意事项”中的编号不再被识别成题目。
- 多图选择题按 OCR 来源顺序绑定题干图和 A-D 选项图，旧题的裸选项标签也能正常显示图片。
- 默认 OCR 改为请求 MinerU；容器未安装时明确显示文字层回退。
- 统一文字与图片审核模型选择，并隔离超出原题 A-D 的结构化选项。

## [0.16.0] - 2026-08-16

### Changed

- 测试和运行审计等仓库内辅助脚本统一使用新的后端分层导入路径；旧路径继续作为外部兼容入口保留。
- 后端按 API 路由、应用服务、领域契约、Runtime、文件基础设施和 Store 分层；旧导入路径继续兼容，现有行为和测试命令不变。

## [0.15.0] - 2026-08-16

### Changed

- 本地开发启动会先检查 Node.js 运行基线，并给出切换到受支持版本的中文提示，减少 Vite 启动失败。
- 内容生产操作现在返回不可变运行与题目修订审计摘要，便于确认实际生成、审核和 OCR 配置。
- 后端测试集中到 `apps/api/tests/`，测试命令和 CI 入口保持不变，便于学习和维护。

### Added

- 从 FastAPI OpenAPI Schema 自动生成并校验前端 API 基础类型。

### Fixed

- 题干、选项、题目条件和学生入口统一按来源顺序渲染公式与 Markdown 图片，避免图片路径显示为文字或题干图与选项图错位。
- 归档错题时清理旧陪练线程和消息，同时保留错题及作答证据；恢复后会从新线程开始。

## [0.14.1] - 2026-08-15

### Fixed

- 学生提交正确答案后会进入下一道未完成题，完成整套试卷后显示明确的完成态。
- 刷新或重新打开互动试卷时恢复每道题最近一次答案和判定，已提交题目继续显示“重新提交”。
- 学生作答阶段不再主动请求 TTS；只有明确请求讲解时才播放语音。

## [0.14.0] - 2026-08-14

### Added

- 内容生产端新增独立的错题陪练模型切换，可在 Codex、Ollama 和 Mock 之间选择，不会影响题目生成与审核模型。

### Fixed

- 错题掌握验证答错后可修改同一道题并重新提交，答对一次即可完成验证，不再被迫生成第二道题。
- 学生确认“准备好了”后进入变式练习，首道变式题答对后进入掌握验证，普通追问不会让陪练阶段回退。
- 归档错题时清理对应的陪练对话上下文；题目和学习证据仍保留，重新进入时从新对话开始。

## [0.13.0] - 2026-08-14

### Changed

- 题目、选项、题目条件、错题讲解和分步讲解统一使用同一套公式渲染，历史数据中的常见 LaTeX 转义也会自动兼容。
- 内容生产端的讲解画布由 Canvas 负责图形、HTML 负责文字和公式，避免公式显示为原始 `$...$` 文本。

### Fixed

- 修复 KaTeX 内部嵌套元素被题目条件样式误套成多层胶囊的问题。
- 修复学生端恢复已提交题目后选项、思路和提交按钮状态丢失的问题；已有答案会显示为“重新提交”。

## [0.12.0] - 2026-08-13

### Added

- 错题陪练每轮现在会生成可审计的教学计划，并按学生确认的错误原因选择对应教学策略。
- 错题陪练现在可以区分八类学生意图，并在同一次回复中记录带证据和置信度的误区假设。

### Changed

- 连续请求到相似提示时会有界升级提示策略；模型无法覆盖确定性判题或越权推进辅导阶段。
- 每轮陪练只执行一个细粒度教学动作；缺少学生原文证据的误区会自动降级为确认问题。

### Fixed

- “修复本题”现在只替换当前题，不会误删同批次其它题；整批重生成与显式刷新 OCR 仍保留为独立操作。
- 重新生成题目时会创建新的题目修订，并在审核后按 OCR 来源顺序恢复题干图和 A-D 选项图，避免文件名文本、错位图片和旧题假更新。
- PDF 中无法直接提取的几何矢量图、统计图等页面图形会按视觉提示渲染并绑定回题目，避免学生端只看到“如图”文字。
- 错题列表、确认页和陪练页统一清理历史图片 Markdown，避免旧题再次把图片路径显示成题干文字或重复渲染。

## [0.11.0] - 2026-08-13

### Added

- 对缺少独立图片资源的 PDF 页面增加页面渲染回退，保留几何题、统计图题的视觉证据。

### Changed

- OCR 题目生成增加来源图片重绑定和公式/题型质量校验，重新生成会创建独立题目修订。

### Fixed

- 修复旧题将 Markdown 图片路径显示为选项文字的问题，兼容历史保存的题干图与选项图。
- 修复审核模型返回后图片归属被覆盖，导致题干图、选项图错位或丢失的问题。

## [0.10.0] - 2026-08-12

### Added

- 内容生产者可以一次加入多个教材 PDF 或图片，在同一队列中查看每个文件的独立上传、OCR 识别和生成进度。

### Changed

- 教材识别最多并行处理三个文件；单个文件可暂停、继续、移除或重试，不会切换时覆盖其他文件的状态。
- 课程语音改为按当前步骤按需预取；切换题目、步骤或发起新语音时会取消尚未返回的旧 TTS 请求，减少本地语音服务阻塞。

## [0.9.0] - 2026-08-11

### Added

- 整本 PDF 会按页面特征选择文字层或 MinerU，缓存中间结果，并只对扫描、公式损坏或质量不合格的局部页段升级识别。

### Changed

- OCR 题目切分支持更多中文题号与跨页续题，已知摄氏度、百分号和 LaTeX 转义损坏会在生成前标准化并进入质量门禁。

### Fixed

- 模型调用失败或读取旧数据时不再把几何示例题和 `show-triangles` 引导卡误用到普通题目。
- 迁移数据卷后错题原图可按错题目录重新定位；辅导页的主提交按钮恢复深色文字对比度。

## [0.8.0] - 2026-08-11

### Added

- 学生在互动试卷中答错或部分答对时会自动加入错题本，离线补传和网络重试不会重复创建记录。

### Fixed

- 复习进度页将 1、3、7 天复习任务与知识点掌握改为纵向阅读布局，减少桌面端左右跳读。
- 学生答题页不再显示重新生成、OCR、审核、模型和内部画布动作；分步讲解仅在请求提示或答错后出现。
- 学生从不同学习子页返回时使用明确且一致的“返回学生空间 / 返回错题本 / 返回教材库”层级。

## [0.7.0] - 2026-08-09

### Added

- 学生完成互动试卷作答后会立即看到当前知识点掌握度、答对次数和累计练习次数。
- 题型结构校验失败时会自动局部修复当前题，并在发布时隔离无法恢复的异常题。
- 内容生产端可独立选择文字审核模型，不再与题目生成模型绑定。
- 已送审或已发布试卷可从原 PDF 整套重新生成审核新版，旧版本和学生学习记录保持不变。
- 内容生产页刷新或重新打开教材时会恢复最新试卷版本，不再丢失送审和发布状态。

### Changed

- 本地 Qwen3-TTS 会在服务启动时提前完成一次语音推理，降低学生首次播放讲解时的等待时间。
- 学习会话明确绑定整份互动试卷，并保留已有会话和离线作答数据。
- 互动试卷不再因单道异常题永久阻塞；合格题可继续安全发布，整套失败时返回开发诊断。
- 短文本选择题使用更接近纸质试卷的紧凑响应式布局，窄屏下自动恢复单列显示。

### Fixed

- 修复 `A.`、`A．` 等常见试卷选项标记未被拆分，导致题干重复选项、结构化选项只剩字母的问题。
- 修复审校模型产生的百分号和摄氏度异常 LaTeX，并阻止题干百分比与温度选项等单位语义冲突进入发布结果。

## [0.6.0] - 2026-08-09

### Added

- 内容生产端可以将多道题送审并发布为互动试卷，学生端可查看已发布试卷并继续作答。
- 学生作答记录支持本地离线排队、批量补传和幂等重试，不会因网络抖动重复累计掌握度。

### Changed

- 内容生产工作台的答题仅用于质量预览，不再把编辑者试做记录为学生掌握度。

### Fixed

- 本地数据库重建后再次打开互动试卷会自动恢复学习会话，离线作答仍保留真实时间和顺序。

## [0.5.0] - 2026-08-09

### Added

- 新增独立学生学习空间，统一进入互动试卷、个人错题本和掌握复习，学生端不再展示教材上传与模型配置。

### Changed

- 教材上传、OCR、模型选择和互动预览统一迁移到内容生产工作台，旧教材地址继续兼容跳转。

## [0.4.0] - 2026-08-08

### Added

- 错题完成单题陪练后，可按错误原因生成基础、同类和迁移三个层级的变式验证题，并保存每次确定性判题结果。
- 连续答对两道不同变式题后，错题会自动进入独立的进阶本，并显示可追溯的掌握进度。
- 掌握错题后自动建立第 1、3、7 天可作答复习任务，并提供掌握率、复习正确率和知识点进度页。

### Changed

- 将教材导入与学习记录的数据库操作拆分为独立领域 Store，同时保留兼容入口，便于分别维护和测试。
- 将 PDF 合并、OCR、出题和批次状态编排移入可复用服务，HTTP 路由只保留传输层职责。

## [0.3.0] - 2026-08-08

### Added

- 增加教材互动学习与 AI 错题陪练两个独立入口。
- 支持拍照或选择错题图片、裁切识别范围、修正确认题目归类，并保存错误原因。
- 为每道已确认错题增加可恢复的独立辅导线程和持久化消息历史。
- 增加定位卡点、解释误区、引导练习和准备验证四阶段多轮陪练。
- 多轮陪练复用选择、多选、判断、填空、数值和画线结构化作答与确定性判题。
- 教材库支持内容去重、软删除和返回导航。
- 练习页面显示当前使用的模型。

### Changed

- 使用 React Router 统一导航，并按产品入口延迟加载页面资源。
- 将前端 API 与类型按教材、错题、辅导、运行时和学习领域拆分，同时保留兼容门面。
- 将数据库配置、关系表声明、教材库路由和批次题目处理从大文件拆为可独立阅读的模块。
- 多轮提示只携带状态摘要和最近必要消息，避免上下文无限增长。
- 缓存并预取课程语音，使讲解音频与步骤切换更稳定。

### Fixed

- 修复模型结构化输出缺少必填字段时请求失败的问题。
- 修复真实模型生成路径缺少画布动作常量的问题。
- 修复数据库环境变量不完整时静默连接到错误实例的问题。
- 支持通过 `CODEX_COMMAND` 指定 Codex CLI 路径。
- 拒绝看似包含结构化字段、实际没有任何作答内容的空提交。

## [0.2.0] - 2026-08-02

### Added

- 增加多选题、填空题和数值/公式题的结构化生成与交互作答。
- 增加填空答案、数值容差和多选集合的确定性核对，并保留模型辅导回退。
- 增加后端 JSON 结构化日志、请求 ID、耗时记录和教材/OCR/模型/TTS/数据库关键事件日志。
- 增加基于 `POSTGRES_*` 环境变量自动组装密码连接串的本地配置方式。
- Docker PostgreSQL 默认映射到宿主机 `127.0.0.1:15432`，便于本地安全访问。
- 增加可编程课程内容块和可扩展渲染器，可组合文字、公式、图形、动画、标注、练习和提示。
- 增加学习会话、作答记录和知识点掌握度，学生提交后可看到最新掌握度反馈。

### Changed

- 将题目契约、题目规范化/质量门禁、答案核对和运行时路由从 FastAPI 入口拆分为独立模块。
- 将题型作答渲染和题目展示逻辑拆分为独立前端组件，便于继续增加题型。
- 将 FastAPI 应用初始化、上传状态、辅导检查和学习接口拆分为独立模块，降低单文件修改风险。

## [0.1.0] - 2026-08-01

### Added

- 增加选择题、判断题、简答题和画线题的统一结构与交互。
- 增加 PDF 分块上传、暂停续传、批次 OCR、来源绑定和教材恢复。
- 增加文本/视觉双模型审校、结构质量门禁和题目来源记录。
- 增加 Qwen3-TTS、Azure Speech Neural 和浏览器语音回退。
- 增加 PostgreSQL/JSONB 存储层和旧 SQLite 幂等迁移工具。
- 增加完整 Docker Compose 部署：PostgreSQL、FastAPI、React/Nginx、健康检查和持久化卷。
- 增加后端/前端 Dockerfile、Nginx 同源代理、Docker 环境模板和 `.dockerignore`。
- 增加 GitHub Actions 后端测试、前端构建和 Docker Compose 健康检查。
- 增加 Apache-2.0 许可证、贡献指南、安全策略、行为准则和支持说明。
- 增加 Issue/PR 模板、CODEOWNERS、Dependabot 和私有漏洞报告入口。
- 增加系统架构、本地开发、API、部署、路线图和模型评估文档。

### Fixed

- 修复模型将 `(A) ... (B) ... (C) ... (D) ...` 合并为单个选项的问题，并增加回归测试。
- 修复项目移动后旧持久化绝对路径导致题图无法恢复的问题。

### Changed

- 本地和生产默认数据库从 SQLite 切换为 PostgreSQL `dotty_tutor`。
- CORS 和可信 Host 改为环境变量配置，API 增加安全响应头、请求 ID 和数据库健康检查。
- README 从内部交付式长文档重构为开源项目首页，详细内容拆分到 `docs/`。
- FastAPI 标题、前端包名和页面副标题统一为 `Dotty Tutor`。
- Docker CI 改为构建前后端镜像、启动完整服务并检查 Web/API 健康状态。

### Notes

- Qwen3-TTS 模型权重保存在本地缓存，不提交到仓库。
- Azure、数据库和模型凭据只通过环境变量或密钥管理提供。
- 当前版本是面向本地体验和受控内测的 MVP，公网部署限制见 `docs/roadmap.md`。

[Unreleased]: https://github.com/ningkaikok/dotty-tutor/compare/v0.32.0...HEAD
[0.32.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.31.0...v0.32.0
[0.31.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.30.0...v0.31.0
[0.30.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.29.0...v0.30.0
[0.26.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.25.0...v0.26.0
[0.25.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.24.0...v0.25.0
[0.24.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.23.0...v0.24.0
[0.23.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.22.0...v0.23.0
[0.22.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.21.1...v0.22.0
[0.21.1]: https://github.com/ningkaikok/dotty-tutor/compare/v0.21.0...v0.21.1
[0.21.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.20.0...v0.21.0
[0.20.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.19.0...v0.20.0
[0.19.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.18.0...v0.19.0
[0.18.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.17.0...v0.18.0
[0.17.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.16.0...v0.17.0
[0.16.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.15.0...v0.16.0
[0.15.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.14.1...v0.15.0
[0.14.1]: https://github.com/ningkaikok/dotty-tutor/compare/v0.14.0...v0.14.1
[0.14.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.13.0...v0.14.0
[0.13.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.12.0...v0.13.0
[0.12.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.11.0...v0.12.0
[0.11.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.10.0...v0.11.0
[0.10.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.9.0...v0.10.0
[0.9.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.8.0...v0.9.0
[0.8.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.7.0...v0.8.0
[0.7.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.6.0...v0.7.0
[0.6.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/ningkaikok/dotty-tutor/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/ningkaikok/dotty-tutor/releases/tag/v0.2.0
[0.1.0]: https://github.com/ningkaikok/dotty-tutor/releases/tag/v0.1.0
