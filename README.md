...
学生学习空间只负责错题陪练、掌握验证和复习，不向学生暴露 OCR、模型或上传配置。
...
- 根路径按角色提供 `/mistakes` 学生学习空间与 `/studio` 内容生产工作台。
- 旧 `/learn`、`/textbooks` 地址会分别兼容跳转到 `/mistakes` 和 `/studio`，避免历史书签失效。
...
- 学习会话、作答记录与知识点掌握度的后端能力（当前无前端写入方，随互动试卷接入）。
...
```text
产品首页
  ├─ 学生学习空间（AI 错题陪练）：拍照错题 → 确认归类 → 多轮陪练 → 变式验证 → 复习计划
...
<http://localhost:5174/mistakes>，内容生产端为 <http://localhost:5174/studio>。完整 Docker Compose
仍适合 CI、演示和发布验证：
