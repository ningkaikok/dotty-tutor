...
- 测试要验证业务行为（可观察的输入输出、状态转换），不要验证实现细节（内部方法被调用几次）。
  重构没改行为却让测试大片失败，大概率是测试本身写错了；`assert_called`/`call_count` 这类断言
  几乎总是这个反模式的信号，改成断言业务可观察的输出（返回值、持久化结果、状态转换）。
- 验收级测试先写场景再写实现：用 `user` 开头、Given/When/Then 描述业务行为，让非工程师也看得懂
  在测什么。不要先写完代码，再照着自己的实现反推一份"证明没错"的测试。这条只要求验收级测试，
  不强制纯函数/工具函数单测也套用这个格式。
- 后端测试里的 `unittest.mock` 只允许用在真正的外部边界（模型/OCR/TTS Runtime），且必须在
  `scripts/check_test_discipline.py` 的 `ALLOWED_MOCK_FILES` 里登记；时间相关行为用注入的
  `now=` 参数，不要 `time.sleep()` 之后再断言。这两条由该脚本在 CI 和 pre-commit 里强制检查，
  细节和例外条件见 [`apps/api/tests/README.md`](apps/api/tests/README.md)。
- 覆盖率报告已启用（`coverage`/`@vitest/coverage-v8`），目前只出数字、不设门槛。写测试时该关注的是
  "剩下没覆盖的失败路径为什么没测"，不要为了拉高覆盖率数字堆砌无实际断言的测试。
