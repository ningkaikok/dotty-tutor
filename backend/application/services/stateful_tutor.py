...
class StatefulTutor:
    ...
    Phase-three transitions are deliberately easy to inspect::
        ...
        confirm-ready -------------------------------> practice
        practice --correct--> verify --any--> verify

    Variant questions are generated by the practice router once the thread
    reaches ``practice`` or ``verify``; two consecutive deterministic variant
    answers are required before the mistake is marked mastered.
    """
