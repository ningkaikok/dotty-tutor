...
本模块回答一个问题：模型不可信或不可用时，陪练还能说什么？
- 模型调用失败、返回内容越权（试图改判题结果）或证据不足时，走这里的确定性回退；
  回退回复必须可审计——宁可说"请再检查一遍"，也不能编造一个误区诊断。
- 代数步骤检查（等式冲突等）用正则做粗粒度校验，只用于拦截明显矛盾，
  不承担完整数学推理；精确判题在 answer_evaluator.py。

与 turn_plan.py 的分工：turn_plan 决定"这一轮该做什么动作"（教学计划），
本模块提供计划之外的安全网和工具函数。两者都属于领域纯函数层，不碰存储。
"""
